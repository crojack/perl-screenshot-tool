...
my $CONFIG_DIR = "$HOME/.config/perl-screenshot-tool";
my $DATA_DIR = "$HOME/.local/share/perl-screenshot-tool";
...
make_path($CONFIG_DIR, $LIB_DIR, $SHARE_DIR, $APPLICATIONS_DIR, $LOG_DIR); 

# Create a placeholder log file so the directory isn't empty
print "Creating log directory and placeholder file...\n";
open(my $log_fh, '>', "$LOG_DIR/README.txt") 
    or die "Cannot create log placeholder file: $!";
print $log_fh "This directory contains log files for the Perl Screenshot Tool.\n";
print $log_fh "Log files will be created automatically when the application runs with logging enabled.\n";
close($log_fh);

# Set proper permissions for log directory
system("chmod 755 $LOG_DIR");
...
system("update-desktop-database $HOME/.local/share/applications 2>/dev/null");
...
system("sudo tee $BIN_DIR/screenshot-tool > /dev/null") << "EOF";
$script_content
EOF
system("sudo chmod 755 $BIN_DIR/screenshot-tool");
...
print "Log files will be stored in: $LOG_DIR\n";  # Added log directory info
